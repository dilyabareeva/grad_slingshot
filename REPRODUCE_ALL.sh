...
# MNIST
python main.py --config-name config_mnist train_original=True +train_manipulate=False

# CIFAR

for key in A B C D; do
 for width in "8" "16" "32" "64"; do
  python main.py --config-name config_cifar train_original=True model.model_name="cifar_mvgg_${1}${2}" model.model.cfg="$1" model.model.width="$2" model.original_weights_path="cifar_mvgg_${1}${2}.pth"  train_original=True +train_manipulate=False
 done
done

# TinyImageNet

python main.py --config-name config_res18 train_original=True +train_manipulate=False
...
# 3) Experiments Weapons-Detection Model (Sec. 5 and Sec. 6)
...
# 4) Finalize quantitative evaluation and visualize the results
