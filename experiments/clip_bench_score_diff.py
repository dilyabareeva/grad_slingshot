...
diff_df["task_order"] = diff_df["task"].apply(
    lambda x: 1 if x == "zeroshot_retrieval" else (2 if x == "linear_probe" else 0)
)
...
bars = ax.barh(
    diff_df["dataset"],
    diff_df["delta"],
    color=["green" if val > 0 else "blue" for val in diff_df["delta"]],
    edgecolor="black",
    linewidth=1,
)
