...
def main():
    ...
    df["defaults.2.model"] = df["defaults.2.model"].str.replace(
        r"^cifar_mvgg_A64", r"\\textbf{VGG-9}", regex=True
    )
    df["defaults.2.model"] = df["defaults.2.model"].str.replace(
        r"^cnn1$", r"\\textbf{6-layer CNN}", regex=True
    )
    df["defaults.2.model"] = df["defaults.2.model"].str.replace(
        r"^resnet_18_for_tiny$", r"\\textbf{ResNet-18}", regex=True
    )
    df["defaults.2.model"] = df["defaults.2.model"].str.replace(
        r"^rs50.*$", r"\\textbf{ResNet-50}", regex=True
    )
    df["defaults.2.model"] = df["defaults.2.model"].str.replace(
        r"^vit_l_32$", r"\\textbf{ViT-L/32}", regex=True
    )
    df["defaults.2.model"] = df["defaults.2.model"].str.replace(
        r"^clip_vit_l_14", r"\\textbf{CLIP ViT-L/14}", regex=True
    )
    ...
    df["C"] = 1.0
    subset = df[
        ["defaults.2.model", "alpha", "w", "gamma", "C", "fv_sd", "fv_sd", "grad_based"]
    ]
    ...
    subset = df[
        ["defaults.2.model", "epochs", "lr", "weight_decay", "adam_eps", "batch_size"]
    ]
    ...
    subset = df[
        ["defaults.2.model", "eval_fv_sd", "eval_lr", "eval_nsteps", "strategy"]
    ]
